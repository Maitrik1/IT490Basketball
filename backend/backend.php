...
function show_friends($cookie)
{
	$response = array();
	try {
		$stmt = "SELECT t1.fname, t1.lname, t1.id, t1.email FROM users t1 LEFT JOIN friends t2 ON t2.friend_id = t1.id WHERE t2.friend_id = t1.id AND t2.user_id = :cookie";
...
function no_friends($cookie)
{
	$response = array();
	try {
		$stmt = "SELECT  e.id, e.fname, e.lname FROM    users e WHERE   NOT EXISTS( SELECT  *  FROM    friends d WHERE   d.friend_id = e.id AND d.user_id = :cookie )";
		$params = array(":cookie" => $cookie);
		$result = send_sql_query_to_databse(true, $stmt, $params);
		if (!empty($result)) {
			$response["success"] = true;
...
function create_friends($user_id, $friend_id)
{
	$response = array();
	try {
...
function request_processor($req)
{
	...
	try {
		...
		switch ($type) {
				//showing all friends 
			case "show_friends":
				return show_friends($req['session_id']);
				// creating new friends 
			case "create_friends":
				return create_friends($req['session_id'], $req['friend_id']);
				// all other peoples that are not our friends 
			case "no_friends":
				return no_friends($req['session_id']);
		}
	} catch (Exception $e) {
		//echo the error out to stdout
		echo $e->getMessage();
